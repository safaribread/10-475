...
metadata <- read_delim("metadataFull.tsv")
...
# keep samples from the chinese study,  saliva sample type, and from non smokers and cigarette smokers
metadata = metadata[metadata$Data_Origin == "Chinese_Xue_et_al", ]
metadata = metadata[metadata$Sample_type == "Saliva", ]
metadata <- metadata[metadata$Smoking_status %in% c("C", "EC"), ]
...
metadata = metadata[metadata$`Sample_ID` %in% abun_samples,] #making sure the filtered metadata only includes these samples
...
abundance_daa_results_df <- pathway_daa(abundance = abundance_data_filtered %>% column_to_rownames("pathway"), 
                                        metadata = metadata, group = "Smoking_status", daa_method = "DESeq2")
...
#this line will change for each dataset. 10 represents the number of samples in the filtered abundance table
abundance_desc = abundance_desc[,-c(10:ncol(abundance_desc))] 
...
source("DESeq2_function_china.R")

# Run the function on your own data
res =  DEseq2_function(abundance_data_filtered, metadata, "Smoking_status")
